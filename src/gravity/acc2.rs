...
pub struct AccDot2(pub Vec<[Real; 3]>, pub Vec<[Real; 3]>, pub Vec<[Real; 3]>);
impl AccDot2 {
...
impl<'a> From<&'a mut AccDot2> for DstSliceMut<'a> {
    fn from(acc: &'a mut AccDot2) -> Self {
        DstSliceMut {
            adot0: &mut acc.0[..],
            adot1: &mut acc.1[..],
            adot2: &mut acc.2[..],
...
impl<T: Into<SrcSlice<'_>>> Compute<T> for AccDot2Kernel {
    type Output = AccDot2;
    ...
    impl Distribution<SrcSoA> for Standard {
        fn sample<R: Rng + ?Sized>(&self, rng: &mut R) -> SrcSoA {
            SrcSoA {
                eps: rng.gen(),
                mass: rng.gen(),
                rdot: Derivs(rng.gen(), rng.gen(), rng.gen()),
            }
        }
    }
